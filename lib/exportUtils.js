...
export function exportClientReportToPDF(client, clientPackages, filename = 'client-report.pdf') {
  ...
  doc.text(`Email: ${client.email || 'N/A'}`, 20, 75)
  doc.text(`Phone: ${client.phone || 'N/A'}`, 20, 85)
  doc.text(`Address: ${client.address || 'N/A'}`, 20, 95)
  doc.text(`Created: ${new Date(client.created_at).toLocaleDateString()}`, 20, 105)

  // Packages Table
  if (clientPackages.length > 0) {
    doc.text('Package History', 20, 125)
